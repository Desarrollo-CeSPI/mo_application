...
def mo_mysql_data_for_cluster(cluster_name)
  master = search('node',"chef_environment:#{node.chef_environment} AND tags:mysql_master AND cluster_name:#{cluster_name}")
  host = master.first
  fail "Did not find a MySQL cluster named #{cluster_name} tagged as mysql_master for environment #{node.chef_environment}"  if host.nil?
  { 'host' => host.fqdn, 'type' => 'mysql' }
end
